...
if(interactive()){
  ...
  out_file <- paste0('/data/vrc_his/douek_lab/projects/RNASeq/', project, '/results/', 
                     qc_name, '/DSB_normalized_data.RDS')
  pdf_file <- paste0('/data/vrc_his/douek_lab/projects/RNASeq/', project, '/results/', 
                     qc_name, '/DSB_normalized_data.pdf')
  sdat_files <- paste0('/data/vrc_his/douek_lab/projects/RNASeq/', project, 
                       '/results/', qc_name, '/batches/', batches, '/Cell_filtered.RDS')
  
  mat_files <- paste0('/data/vrc_his/douek_lab/projects/RNASeq/', project,
...
if(length(sdats) == 1){
  ...
  sdat <- JoinLayers(sdat)
  
  # ### Get counts as data.frames. merge doesn't do this right now, but it seems that it will eventually (and it once did) given the 'collapse' parameter that doesn't work if set to T
  # mats <- lapply(sdats, function(sdat) sdat@assays$RNA@layers$counts)
  # ### Merge  counts
  # mat <- mats[[1]]
  # if(length(mats) > 1){
  ...
  # }
  # rm(mats)
  # sdat@assays$RNA@layers$counts <- mat 
  # ### Remove the 'counts.1', 'counts.2', etc.
  # sdat@assays$RNA@layers[paste0('counts.', 1:length(sdats))] <- NULL
}
...
pdf(pdf_file)
VlnPlot(sdat, assay = 'RNA', features = 'nCount_RNA', group.by = batch, pt.size = 0) + theme(legend.position = "none")
if(prot){
