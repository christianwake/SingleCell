...
fgseas_list <- lapply(fgsea_files, function(fgsea_file) read.table(fgsea_file, header = T, check.names = F, stringsAsFactors = F, na.strings = c("", "NA"), sep = '\t'))
...
test_values <- paste0(test_value1, '-', test_value2)
names(test_values) <- names(test_value1)
