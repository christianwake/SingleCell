...
dehash_CR_ID <- function(dat, id, runs_dir){
  ...
  data_dir <- my_read_10x(id, dat, runs_dir, cellranger = 'count', filtered = 'filtered')
  ### Read filtered data
  ...
  rm(CR_filtered)
  ### Get desired directory of data
  data_dir <- my_read_10x(id, dat, runs_dir, cellranger = 'count', filtered = 'raw')
...
if(interactive()){
  ...
  # out_labels <- '/hpcdata/vrc/vrc1_data/douek_lab/projects/RNASeq/2021614_21-002/data/Cell_data.csv'
  ...
  out_labels <- '/hpcdata/vrc/vrc1_data/douek_lab/projects/RNASeq/2022619_857.3b/data/Cell_data.csv'
  out_metrics <- '/hpcdata/vrc/vrc1_data/douek_lab/projects/RNASeq/2022619_857.3b/cellranger_count_metrics.tsv'
  quant_func <- 'count'
  batch <- 'Date_Sort'
...
for(id in unique(dat$CR_ID)){
  label_list[[id]] <- dehash_CR_ID(dat, id, runs_dir)
}
